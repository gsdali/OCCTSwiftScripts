...
enum SolveSketchCommand: Subcommand {
    static let name = "solve-sketch"
    static let summary = "Solve a 2D constraint sketch (SwiftGCS)"
    static let usage = """
        Usage:
          solve-sketch                  (read JSON sketch from stdin)
          solve-sketch <sketch.json>    (read JSON sketch from file)
        Supported constraints: coincident, horizontal, vertical, fixed_distance,
        fixed, parallel, perpendicular, angle, point_on_line, point_on_circle,
    ...
    static func run(args: [String]) throws -> Int32 {
        ...
        if let path = args.first, !path.hasPrefix("-") {
            guard let bytes = FileManager.default.contents(atPath: path) else {
                throw ScriptError.message("Failed to read \(path)")
            }
            data = bytes
        } else {
        ...
        do {
            ...
            throw ScriptError.message("Invalid request JSON: \(error.localizedDescription)")
        }
        ...
        if case .failed = result.status { return 2 }
        return 0
    }
}
