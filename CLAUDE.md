...
- **occtkit** (executable product) — multi-call umbrella binary. Dispatches by `argv[0]` basename (busybox-style symlinks installed by the Makefile) or by first positional arg (`occtkit graph-validate ...`). Each verb lives in `Sources/occtkit/Commands/<Verb>.swift` conforming to the `Subcommand` protocol in `Sources/occtkit/Subcommand.swift`. Verbs: `run` (script-host, replaces standalone OCCTRunner), `graph-validate`, `graph-compact`, `graph-dedup`, `graph-query`, `graph-ml`, `feature-recognize`, `solve-sketch`, `dxf-export`, `drawing-export`. **This is the recommended surface going forward.**

**`drawing-export`** is the multi-view ISO technical drawing exporter. Its support code lives in `Sources/occtkit/Drawing/` (`Spec.swift`, `PaperSize.swift`, `Sheet.swift`, `ViewPlacer.swift`, `MultiViewLayout.swift`, `SectionExtraction.swift`). The verb consumes a JSON spec and produces a single DXF R12 sheet with ISO 5457 border + centring marks, ISO 7200 title block, ISO 5456-2 first/third-angle projection symbol, HLR-projected orthographic views laid out per ISO 128-30, section views (cutting plane → 2D contour in plane frame), cutting-plane lines + section labels, auto-centerlines (via `Drawing.addAutoCentrelines`), and user-specified centermarks + dimensions. Several pieces are DIY'd because OCCTSwift lacks them today — see upstream issues OCCTSwift#73 (section2D), #74 (hatching), #75 (Drawing.transformed), #76 (ISO templates). Once those land, the verb collapses to a thin orchestrator.
